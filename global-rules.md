...
- **Thiết kế các Module Dùng chung dữ liệu (Shared Data Modules Pattern)**: Đối với các module nghiệp vụ có sự chồng chéo hoặc liên quan chặt chẽ đến nhau về mặt thông tin (ví dụ: Nhân sự & Tài xế, Khách hàng & Nhà cung cấp...):
  ...
  - **Kỷ luật Audit Lỗi Toàn Diện (Anti-Superficial Bug Fixing)**: Khi thực hiện sửa lỗi (fix bug), cấm làm việc hời hợt hoặc chỉ sửa chữa phần lỗi bề mặt. Bắt buộc phải thực hiện phân tích luồng liên đới: nếu thay đổi cấu trúc DB hoặc hành động xóa (delete), phải rà soát tất cả các DB triggers, các tác vụ tính toán tự động phụ thuộc để đảm bảo không phát sinh lỗi khóa ngoại gián tiếp do DB triggers hay ràng buộc bất tương thích. Việc sửa lỗi phải đảm bảo "1 phát ăn luôn" thông qua kiểm tra toàn diện chuỗi tác động trước khi báo cáo hoàn tất.
