...
- [ ] structural-map.yaml — `component_mappings` (create/adapt/reuse per component), `routes` object keyed by canonical target route, `state_ownership`, `data_contracts`, `event_flows`
- [ ] visual-contract.yaml — `surfaces` object keyed by canonical kebab-case surface ID, `shell_must`, variable slots, `responsive_breakpoints`
- [ ] behavior-contract.yaml — `behaviors` object keyed by canonical kebab-case surface ID, `behavior_must`, `states_must`, `motion_must`, `responsive_must`, accessibility, interaction flows
- [ ] architecture-adaptation.yaml — `preserve`, `adapt`, `must_not_copy`, `target_equivalents`, `accepted_deviations` object keyed by deviation ID
- [ ] deviations.yaml — `deviations` object keyed by deviation ID, each with `affected_surface`, `changed_invariant`, `rationale`, `proof`
