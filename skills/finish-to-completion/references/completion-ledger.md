...
- Record the two delegation facts from [`rules/10-execution-planning-delegation.md`](../../../rules/10-execution-planning-delegation.md): `delegated` (what went out, to whom, and why) and `outcome` (`consumed`/`rejected` with reason, or `skipped` with reason). The former seven-event receipt chain is retired; every delegated slice must have a recorded `outcome` or an explicit skip reason.
...
For the delegated/resume receipt sequence (the two-fact `delegated`/`outcome` contract), see [`slice-gate-protocol.md`](slice-gate-protocol.md) and its canonical owner [`rules/10-execution-planning-delegation.md`](../../../rules/10-execution-planning-delegation.md).
