...
|-------------|------------------------|
| Owner nói "mới một nửa" | `.agent/ledger/*.md` — đếm `- [ ]` |
...
2. .agent/ledger/<slice-id>.md → ## CONTEXT (nếu có) → đếm - [ ]
...
| Ledger | `.agent/ledger/<id>.md` — AC + verify + evidence; nguồn PASS duy nhất |
| Slice | 1 session, ≤10 AC, 1 subsystem |
...
| **Progress** | Session heartbeat | `.agent/ledger/_progress.md` — phase/slice map |
| **Ledger** | Nguồn PASS duy nhất | `.agent/ledger/<slice-id>.md` — AC + evidence |
...
- **1 session = 1 slice** — ≤10 AC, 1 subsystem/layer.
...
```bash
rg '\[ \]' .agent/ledger/          # phải 0 open AC (trừ [!] blocked)
rg 'evidence: <chưa chạy>' .agent/ledger/
```
...
|-----------------|---------------|--------|
| `LEDGER_PATH` | `.agent/ledger/<plan-id>.md` | Path E hoặc Plan Architect HANDOFF |
| `PROGRESS_PATH` | `.agent/ledger/_progress.md` | Trước phase đầu (`goal-autopilot` Phần 2) |
...
**Phase 2 (spec):** `automation/11-audit-slice-ledger.ps1` — fail nếu report có `Status: PASS` nhưng ledger còn `[ ]`. Chưa implement; H11 harness audit chỉ check wiring.
