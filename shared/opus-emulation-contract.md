...
Áp dụng: `rules/06-*` → sync → `.grok/rules/`, `platforms/codex/rules/`, `.agents/rules/`, `platforms/antigravity/.agents/rules/`.
...
| O12 | **Không plan ảo** — prompt dài/multi-domain phải qua Intent Fidelity + Locked Plan Acceptance trước implement | MEDIUM/HIGH |
| O13 | **Không claim thiếu chứng cứ** — "đã làm/đã test/đã sync/đã deploy" phải có evidence trực tiếp | always |
| O14 | **Prompt dài phải được biên dịch** — rời rạc/multi-domain phải thành requirement graph + source-of-truth map + acceptance contract | MEDIUM/HIGH |
| O15 | **UI phải qua browser** — frontend/admin/public/production không PASS nếu thiếu browser click-through/screenshot evidence | trigger UI |
...
| HIGH | DB, auth, 5fedu UI, production, permission, export, prompt dài/multi-domain | + intent fidelity; locked plan acceptance; full quality matrix; skill/context; không PASS thiếu gate |
...
Sửa file này → sync sang:

- `rules/06-opus-emulation-contract.md` nếu contract được tách thành rule active
- `scripts/sync-all-harness.sh` (Grok + Codex + Antigravity)
- mirror `.agents/rules/` và platform/runtime tương ứng
