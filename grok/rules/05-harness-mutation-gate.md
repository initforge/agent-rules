...
| L2 | Harness master | `grok/rules/`, `grok/skills/`, `grok/INTENT.md` | **Chỉ khi user yêu cầu rõ** |
...
- Cleanup/audit → **không** đụng `grok/`, `.grok/`, `codex/rules/`, protected skills
- Chạy `sync-all-harness.sh` khi user chưa yêu cầu sync harness
...
docs/07-grok-cli-harness.md
docs/08-opus-emulation-harness.md
```

Live `.grok/` là mirror — **sửa master `grok/`**, rồi sync. Không sửa live rồi quên master.
...
4. Chạy `grok/scripts/sync-all-harness.sh` nếu đổi `grok/rules` hoặc `grok/skills`.
...
Ghi vào `plan/` hoặc báo cáo cuối. **Không** ghi thẳng vào `grok/rules/`.
...
| Copy ceremony Antigravity cũ sang Grok | Opus-emulation; xem `docs/06` |