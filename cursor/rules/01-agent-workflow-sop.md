...
Gốc: `codex/rules/01-agent-workflow-sop.md`. Composer: giữ coverage, bỏ ceremony cho task LOW.
...
**Learning loop:** Feedback lặp → ghi L1 (project context) hoặc **Harness proposal** (xem `05-harness-mutation-gate.md`). **Cấm** tự sửa `cursor/`, `.grok/`, `codex/rules/` trừ khi user yêu cầu rõ. Không tự commit.
