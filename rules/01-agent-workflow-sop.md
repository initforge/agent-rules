...
## 5. Kỷ luật giao tiếp, Lập Plan & Quản lý Context (Chống bỏ sót mệnh đề)

*   **Quy chuẩn Tiến hóa Context & Phản xạ Feedback (Context Evolution Gate - BẮT BUỘC KÍCH HOẠT KHI CÓ HÀNH ĐỘNG CẬP NHẬT FILE .MD)**:
    1.  **Điều kiện Kích hoạt (Trigger)**: Agent bắt buộc phải kích hoạt skill [context-evolution-protocol](file:///home/linhnxdeveloper/Projects/nostime/.agents/skills/context-evolution-protocol/SKILL.md) khi có bất kỳ hành động chỉnh sửa, cập nhật hoặc tạo mới nào vào các tệp tin có phần mở rộng `.md` (bao gồm rules lives, spec dự án, workflows, và `implementation_plan.md`), hoặc khi người dùng yêu cầu *"ghi nhớ"*, *"bổ sung context"*, *"đưa vào rule"*, *"đừng lặp lại"*, hoặc khi phát hiện lỗi lặp lại $\ge 2$ lần. Tuyệt đối không được bỏ qua.

    2.  **Cấm viết bậy/đặc thù**: Tuyệt đối không nhét trực tiếp feedback thô, ví dụ thực tế hoặc đặc tả tính năng cụ thể (tên file, tên module, tên cột dữ liệu như `ton-kho.tsx`, `Tổng-Còn-Đã bán`) vào các rules lives chung của Agent (`.agents/rules/` và `agent-rules/rules/`).
    3.  **Trừu tượng hóa & Định tuyến đúng tầng**:
        *   *Tư duy/Nguyên lý toàn năng*: Trừu tượng hóa feedback thành nguyên lý thiết kế, engineering pattern hoặc quy trình SOP áp dụng cho mọi case rồi ghi vào tệp rules lives chung (VD: `08-ui-consistency-gate.md`, `01-agent-workflow-sop.md`).
        *   *Đặc tả nghiệp vụ cụ thể*: Ghi nhận các thông số cụ thể của module vào tệp quy chuẩn cục bộ của dự án (VD: `.agents/5fedu/03-ui-ux-and-delivery-standards.md`, `.agents/5fedu/06-decision-status.md`).
        *   *Tra cứu bản đồ*: Tra cứu tệp chỉ mục [00-index.md](file:///home/linhnxdeveloper/Projects/nostime/.agents/rules/00-index.md) trước khi sửa rules để định vị chính xác tệp tin chịu trách nhiệm, tránh làm loạn hoặc phân bổ sai lệch context.
    4.  **Bảo toàn tri thức**: Khi cập nhật rules lives hoặc plan, giữ nguyên 100% ngữ cảnh cũ, cấm xóa đè cẩu thả. Đồng bộ (sync) ngay lập tức sang repo `agent-rules` sau khi chỉnh sửa.
...
*   **Cập nhật Plan lập tức (Real-time Plan Update)**: Trong quá trình nói chuyện và bàn luận, bất kỳ khi nào người dùng làm rõ một yêu cầu, thay đổi thiết kế hoặc chốt phương án mới, Agent phải **cập nhật ngay lập tức** các nội dung này vào tệp [implementation_plan.md](file:///home/linhnxdeveloper/.gemini/antigravity/brain/6895aae9-21c9-43a8-b4db-71cab295d352/implementation_plan.md) ngay trong lượt gọi đó. Nghiêm cấm việc thảo luận suông mà quên cập nhật vào bản kế hoạch chính thức.
