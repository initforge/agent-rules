...
3.  **Ghi nhận vật lý bắt buộc (MEDIUM/HIGH task)**: Bắt buộc phải tạo hoặc cập nhật danh sách các hạng mục Scope Lock này vào tệp tin vật lý `task.md` (nằm trong thư mục artifact hoặc cấu hình tương ứng). Cấm chỉ khóa scope nhẩm trong đầu.
4.  **Tuyệt đối không** tự ý vẽ thêm việc hoặc mở rộng scope (ví dụ: cố gắng tối ưu hóa toàn bộ hệ thống hoặc đóng mọi khoảng trống không liên quan) trừ khi người dùng yêu cầu rõ ràng.
5.  Nếu bắt buộc phải mở rộng scope trong quá trình làm việc $\rightarrow$ thông báo và xin ý kiến người dùng **trước khi làm** — cấm im lặng làm rồi vứt đống backlog/GAP ở cuối turn.
...
*   `[ ]` **Bằng chứng kiểm thử bắt buộc (Verification proof)**: Báo cáo kết quả verify phải trích xuất tối thiểu 5 dòng đầu và 5 dòng cuối của output/logs chứa kết quả thực tế (ví dụ: `PASSED`, `FAILED`), cấm chỉ mô tả chung chung hoặc fake logs.
...
*   **Hành vi bị cấm:**
    ...
    *   Tự giải thích/ngụy biện lỗi verify thất bại là "lỗi môi trường cục bộ" mà không chứng minh bằng câu lệnh chạy thực tế để debug môi trường.
