...
3. Nếu `/template` không có mẫu trực tiếp, dùng golden reference gần nhất trong app, mặc định là `features/he-thong/nhan-vien` cho list/detail/form/table/toolbar.
4. So sánh template/golden reference với code hiện tại trước khi sửa.
5. Báo cáo cuối phải có dòng `Template checked` hoặc nêu rõ vì sao không kiểm được.
