...
- `/5fedu-project` là để scaffold hoặc duy trì context project-local: `AGENTS.md` và `.codex/5fedu/*.md`. Không tự nhảy sang implement feature nếu user chỉ yêu cầu setup/context.
- `/codex-research` là để gom bằng chứng trước khi quyết định. Tách rõ fact, guess, risk, next steps và nguồn.
- `/runtime-sync-audit` là để so runtime `C:\Users\DELL\.codex` với backup `P:\agent-rules\codex`. Không ghi đè `config.toml` trừ khi user yêu cầu rõ.
