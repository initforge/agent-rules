...
export async function doctor(
  ...
  const allPlatforms = ["codex", "grok", "antigravity", "cursor"] as const;
  ...
  for (const name of platforms) {
    ...
    const buildDir = path.join(root, "05-generated", "runtime-build", name);
    ...
    // Native activation
    const nativeCliMap: { [key in PlatformName]: string } = { codex: "codex", cursor: "cursor", grok: "grok", antigravity: "gemini" };
    report.push({
    ...
    const hookConfigPath = path.join(runtimeHome, {
      ...
      antigravity: "hooks.json", cursor: "hooks.json",
    }[name] || "hooks.json");
