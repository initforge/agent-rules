...
describe("Configuration", () => {
  ...
  it("tsconfig.json exists with strict mode", () => {
    const tsconfig = JSON.parse(
      fs.readFileSync(path.join(root, "cli", "tsconfig.json"), "utf-8")
    );
  ...
  it("source files exist for all commands", () => {
    ...
    for (const cmd of commands) {
      const file = path.join(root, "cli", "src", "commands", `${cmd}.ts`);
      expect(fs.existsSync(file)).toBe(true);
    }
  });

  it("adapter file exists", () => {
    const adapter = path.join(root, "cli", "src", "adapters", "powershell.ts");
    expect(fs.existsSync(adapter)).toBe(true);
  });

  it("main entry point exists", () => {
    const entry = path.join(root, "cli", "src", "index.ts");
