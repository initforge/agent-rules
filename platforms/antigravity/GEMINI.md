...
- **Mandatory Tooling**: Phải ưu tiên sử dụng các công cụ phân tích cấu trúc được liệt kê trong [Open-source Tools Registry](../../docs/09-opensource-tools-registry.md) (đặc biệt là `codebase-memory-mcp`) để loại bỏ điểm mù kiến trúc. Tuyệt đối không phụ thuộc vào các công cụ nén token shell như `RTK` hay `Caveman` của Codex.
