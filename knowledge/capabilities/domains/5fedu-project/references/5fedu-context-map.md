...
- Project layer: repo-local `AGENTS.md` plus canonical `context/5fedu/*.md`. `.agents/AGENTS.md` and `.codex/AGENTS.md` are thin pointers only; never mirror the full pack into platform folders.
...
Every 5fedu repo should track decisions in `context/5fedu/04-decision-status-and-backlog.md`; when legacy expanded files exist, keep `context/5fedu/06-decision-status.md` synchronized too.
