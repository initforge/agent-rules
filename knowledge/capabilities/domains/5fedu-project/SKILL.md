...
1. Inspect the target repo and existing `AGENTS.md`/`context/00-index.md`.
...
- Read `context/5fedu/00-index.md`, decision status and questions first.
...
Update the canonical context, decision status and index. Verify platform pointers still resolve to `context/5fedu/`; do not create `.agents/5fedu` or `.codex/5fedu` mirrors.
