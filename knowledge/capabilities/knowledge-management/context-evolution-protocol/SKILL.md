...
- the task modifies or audits `AGENTS.md`, `knowledge/**`, `integrations/**`, `platforms/**`, `.agents/**`, `.codex/**`, project context, or runtime harness files;
...
| `global-agent-rule` | Applies across projects/platforms | `knowledge/core/**` or a shared capability |
| `raw-evidence` | Chat/log/example kept for traceability | archive/backlog, not default load |
| `question` | Missing owner/spec decision | `questions.md` or decision backlog |
