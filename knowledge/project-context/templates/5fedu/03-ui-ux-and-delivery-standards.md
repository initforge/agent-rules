...
2.  **Cổng 2: Non-Browser Functional Evidence**:
    - Với Codex, không dùng browser subagent/Playwright mặc định. Xác minh CRUD/flow bằng source trace, unit/integration tests, service/API checks, DB query, cache invalidation review, generated artifact inspection và build output.
    - Nếu một flow UI chỉ có thể chứng minh bằng click-through thật, ghi `PARTIAL` với browser verification gap trừ khi user yêu cầu rõ dùng browser.
3.  **Cổng 3: Verify-on-Production (Opt-in cho Codex)**:
    - Sau khi CI/CD tự động deploy lên Vercel, production browser verification chỉ chạy khi user yêu cầu rõ. Nếu không, Codex báo bằng chứng non-browser đã chạy và phần production/browser chưa kiểm.
...
Với 5fedu, production là môi trường verify mặc định sau khi user đã yêu cầu push và CI/CD deploy xong, nhưng Codex không mở browser/Playwright mặc định. Không tự push chỉ để test production nếu user chưa yêu cầu rõ.
...
- Chỉ chạy Playwright/browser trên project production khi user yêu cầu rõ hoặc platform overlay cho phép; tuyệt đối không tự ý deploy thủ công từ terminal bằng lệnh `vercel --prod`.
