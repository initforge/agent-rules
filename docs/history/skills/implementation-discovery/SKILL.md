...
description: Reality-check only when a shared behavior, plan assumption, schema, permission, template or runtime detail may
  be wrong. Trigger on "verify trước khi sửa", "known-unknowns", stack vs spec, migration/auth/permission uncertainty, mojibake,
  orphaned records, stale closure or template gaps. Do NOT use for an obvious one-file fix, pure Q&A or plan-only work.
---

# DEPRECATED — use engine discovery

This skill is superseded by the engine's built-in discovery behavior. The SKILL.md is retained for reference only; new routing should not target it.
...
Ví dụ: `context/<profile>/domains/ui-delivery.md`, `context/<profile>/domains/ui-delivery-detail.md`, `context/<profile>/domains/module-mapping.md`.
