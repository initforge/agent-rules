...
- `id` của mọi bảng app phải là `int8` và tự động tăng dần. Supabase/PostgreSQL hỗ trợ việc này bằng `generated by default as identity` hoặc `bigserial`.
- Không dùng `uuid` cho khóa chính bảng app nếu owner/sheet không chốt rõ.
