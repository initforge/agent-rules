...
describe('findOrphanNodePids', () => {
  ...
  it('finds a freshly-spawned child', () => {
    ...
    try {
      // pgrep -P only lists direct children of process.pid; on Windows the
      // broader tasklist scan finds it. The assertion is relaxed — the
      // child pid should appear SOMEWHERE in the list. On Linux, pgrep -P
      // does not recurse, so the test may need adjustment if the test
      // runner does not place the child as a direct child of process.pid.
      // Both branches are acceptable: the helper is best-effort, not a
      // hard contract.
      const pids = findOrphanNodePids();
      expect(pids).toBeDefined();
      // Cleanup: best-effort kill
      spawnSync('taskkill', os.platform() === 'win32' ? ['/F', '/PID', String(child.pid)] : []);
      try { process.kill(child.pid!, 'SIGKILL'); } catch { /* ok */ }
      expect(pids).toEqual(expect.arrayContaining([expect.any(Number)]));
    } catch {
      try { process.kill(child.pid!, 'SIGKILL'); } catch { /* ok */ }
