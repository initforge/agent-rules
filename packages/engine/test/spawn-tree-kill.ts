...
export function findOrphanNodePids(): number[] {
  if (os.platform() === 'win32') {
    const parse = (stdout: string): number[] => stdout
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter((line) => /^\d+$/.test(line))
      .map((line) => Number(line))
      .filter((pid) => pid !== process.pid && pid !== process.ppid);
    // WMIC is absent on current Windows runners; keep it as a compatibility
    // path and use the inbox CIM provider as the authoritative fallback.
    const wmic = spawnSync('wmic', ['process', 'where', `ParentProcessId=${process.pid}`, 'get', 'ProcessId'], { encoding: 'utf8' });
    if (wmic.status === 0) return parse(wmic.stdout ?? '');
    for (const shell of ['pwsh', 'powershell']) {
      const ps = spawnSync(shell, [
        '-NoProfile', '-NonInteractive', '-Command',
        `(Get-CimInstance Win32_Process -Filter 'ParentProcessId=${process.pid}' | Select-Object -ExpandProperty ProcessId) -join [Environment]::NewLine`,
      ], { encoding: 'utf8', windowsHide: true });
      if (ps.status === 0) return parse(ps.stdout ?? '');
    }
    return [];
  }
