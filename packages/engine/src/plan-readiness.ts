...
export function compileRequirements(
  ...
  for (const mr of m11Reqs) {
    out.push({
      ...
      notes: ['additive M11 requirement; no implementation or evidence yet'],
    });
  }
...
function clusterForM11(id: string): string {
  if (id === 'M11-R11' || id === 'M11-R12') return 'C1';
  if (id === 'M11-R13' || id === 'M11-R14') return 'C2';
  if (id === 'M11-R15') return 'C3';
  if (id === 'M11-R16') return 'C4';
  if (id === 'M11-R17') return 'C5';
  if (id === 'M11-R18' || id === 'M11-R19') return 'C6';
  if (id === 'M11-R20' || id === 'M11-R21') return 'C7';
  if (id === 'M11-R22' || id === 'M11-R23') return 'C8';
  if (id === 'M11-R24' || id === 'M11-R25') return 'C9';
  if (id === 'M11-R26') return 'C10';
  return 'C10';
}
...
function buildSystemTopologyYaml(): Record<string, unknown> {
  // Honest stub per AM-0019 §8. GAP markers where the component does not exist yet.
  return {
    schema_version: 1,
    services: [
      ...
      { id: 'autopilot-supervisor', kind: 'process', status: 'GAP', note: 'host-neutral durable supervisor not yet implemented' },
      { id: 'integration-train', kind: 'process', status: 'GAP', note: 'rolling train merge owner not yet implemented' },
    ],
    ports: [
      { service: 'control-plane', port: 8787, host: '127.0.0.1', note: 'local-only; origin/host checks enforced' },
      { service: 'engine-mcp', port: 'TBD', host: '127.0.0.1', note: 'GAP — engine broker port unassigned' },
    ],
    ...
    queues: [{ id: 'ready-queue', status: 'GAP', note: 'cross-stage typed ready queue from AM-0019 §4' }],
    migrations: [
    ...
    journeys: [
      ...
      { id: 'full-stack-public-ingress', status: 'GAP', note: 'requires exact deployed topology (AM-0019 §8)' },
    ],
    rollback: {
...
function buildExecutionGraphYaml(): Record<string, unknown> {
  ...
  return {
    ...
    stages: [
      ...
      { id: 'C1', name: 'plan-readiness', state: 'IN_PROGRESS', note: 'this C1 deliverable' },
      { id: 'C2', name: 'typed-execution-graph', state: 'GAP' },
      { id: 'C3', name: 'native-swarm-scheduling', state: 'GAP' },
      { id: 'C4', name: 'worktree-isolation-integration-train', state: 'GAP' },
      { id: 'C5', name: 'global-resource-tool-broker', state: 'GAP' },
      { id: 'C6', name: 'durable-autopilot', state: 'GAP' },
      { id: 'C7', name: 'system-topology-verification', state: 'GAP' },
      { id: 'C8', name: 'browser-parity-visual', state: 'GAP' },
      { id: 'C9', name: 'host-convergence-certification', state: 'GAP' },
      { id: 'C10', name: 'terminal-release', state: 'GAP' },
    ],
...
export function compilePlanReadiness(input: PlanReadinessInput): PlanReadinessResult {
  ...
  const requirements = compileRequirements(ledger, input.amendmentPath, input.originalPath);
  ...
  const documents: Record<string, unknown> = {
    ...
    'execution-graph.yaml': buildExecutionGraphYaml(),
