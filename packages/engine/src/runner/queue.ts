...
export class TaskQueue {
  ...
  recoverAbandoned(): QueuedTask[] {
    ...
    for (const name of fs.readdirSync(activeDir).filter((n) => n.endsWith('.json'))) {
      ...
      // Check if the process that claimed this task is still alive
      if (task.claimedAt) {
        const claimedAge = now - new Date(task.claimedAt).getTime();
        // If claimed recently (< 30 min), check if PID is alive
        // (We store a simple process marker in claimedAt; the actual PID check
        //  is done by the journal lock recovery in the runner startup.)
        if (claimedAge < STALE_THRESHOLD_MS) {
          // Too recent to assume dead — skip recovery
          continue;
        }
      }
