...
import { captureDelta, snapshotDirtyFiles, isDocOnly } from './diff.js';
...
export class Runner {
  ...
  private async runTask(task: QueuedTask): Promise<TaskReport> {
    ...
    // Capture dirty file snapshot BEFORE agent execution.
    const runnerExclusions = this.runnerOwnedPaths();
    const beforeFiles = snapshotDirtyFiles(this.config.cwd, runnerExclusions);

    const execution = await this.executor.execute(task);

    // Capture dirty file snapshot AFTER agent execution.
    const afterFiles = snapshotDirtyFiles(this.config.cwd, runnerExclusions);

    // Compute true delta: only NEW changes introduced by this agent.
    const diff = captureDelta(this.config.cwd, beforeFiles, afterFiles, task.ownedPaths);
    ...
    // BUG FIX #1: Check agent exit code BEFORE verification.
    // If the agent process itself failed (exit ≠ 0), this is NOT a pass regardless
    // of what the verification commands say. The agent didn't complete its work.
    if (execution.exitCode !== 0) {
      const reason = execution.timedOut
        ? `agent timed out after ${execution.durationMs}ms`
        : `agent exited with code ${execution.exitCode}`;
      this.journal.append('AGENT_FAILED', { taskId: task.id, exitCode: execution.exitCode, reason });
      return this.handleFailure(task, reason, execution);
    }

    // BUG FIX #3: Check ownership violations.
    if (diff.ownershipViolations.length > 0) {
      const reason = `ownership violation: agent edited files outside ownedPaths: ${diff.ownershipViolations.join(', ')}`;
      this.journal.append('OWNERSHIP_VIOLATION', {
        taskId: task.id,
        violations: diff.ownershipViolations,
      });
      return this.handleFailure(task, reason, execution);
    }
    ...
    const reason = `verification failed (exit codes: ${verificationExitCodes.join(', ')})`;
  ...
  private handleFailure(task: QueuedTask, reason: string, execution: { exitCode: number; timedOut: boolean; durationMs: number }): TaskReport {
    const base: Omit<TaskReport, 'outcome' | 'reason'> = {
      taskId: task.id,
      exitCode: execution.exitCode,
      durationMs: execution.durationMs,
    ...
    const repair = this.queue.add({
      prompt: this.repairPrompt(task, reason, []),
      verification: task.verification,
      ownedPaths: task.ownedPaths,
      repairDepth: task.repairDepth + 1,
      requirementId: task.requirementId,
