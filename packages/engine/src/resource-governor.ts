...
function readProcMeminfo(): { memAvailable: number; memTotal: number; swapAvailable: number; swapTotal: number } {
  try {
    ...
    // macOS and Windows do not expose Linux /proc/meminfo. Keep the native
    // governor usable on those hosts with OS-provided totals; swap counters
    // remain unknown rather than being fabricated.
    const memTotal = os.totalmem();
    const memAvailable = os.freemem();
    return { memAvailable, memTotal, swapAvailable: 0, swapTotal: 0 };
  }
}
...
function readCpuLoad(): number {
  try {
    ...
    if (!Number.isFinite(oneMinute)) return 0;
    return Math.min(100, Math.max(0, Math.round((oneMinute / cpuCount) * 100)));
  }
}
...
export class NativeResourceGovernor implements ResourceGovernorAdapter {
  ...
  async detect(): Promise<{ available: boolean; platform: string }> {
    ...
    const platform = `${process.platform}-native`;
    return {
      available: mem.memTotal > 0 && cpu.cpuCount > 0,
      platform: mem.memTotal > 0 && cpu.cpuCount > 0 ? platform : `${platform}-unavailable`,
    };
  }
...
export function createResourceGovernor(effectiveIdentity: Sha256): ResourceGovernorAdapter {
  ...
  // Native means “use host telemetry/process controls”, not “Linux /proc
  // only”. The governor has OS fallbacks for memory/load; unknown hosts stay
  // portable.
  if (['linux', 'darwin', 'win32', 'freebsd'].includes(process.platform)) {
