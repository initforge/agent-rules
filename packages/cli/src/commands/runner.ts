...
function currentPlan(basePath: string): { planId: string; planRoot: string; requirements: string } {
  const ptr = readCurrentPointer(basePath);
  if (!ptr) {
    throw new Error('no active plan: .agent/current.json is missing (see .agent/README.md)');
  }
  const requirements = fs.existsSync(path.join(basePath, ptr.plan_root, 'requirements.yaml'))
    ? path.join(basePath, ptr.plan_root, 'requirements.yaml')
    : path.join(basePath, ptr.plan_root, 'progress.md');
  return {
    planId: ptr.plan_id,
    planRoot: path.join(basePath, ptr.plan_root),
    requirements,
  };
}
