...
const run = (file, args, cwd = repo, options = {}) => {
  try {
    return execFileSync(file, args, { cwd, env, encoding: "utf8", stdio: options.stdio ?? "pipe" });
  } catch (error) {
...
try {
  ...
  const pack = (directory) => {
    const result = JSON.parse(run("npm", ["pack", "--json"], directory));
    return Array.isArray(result) ? result[0] : Object.values(result)[0];
  };
  run("npm", ["run", "build", "-w", "packages/engine"], sourceRoot);
  run("npm", ["run", "build", "-w", "packages/cli"], sourceRoot);
  ...
  try {
    run("npm", ["init", "-y"], app);
    run("npm", ["install", engineTar, cliTar], app);
    const homeBefore = await fsp.readdir(home).catch(() => []);
    ...
    await fsp.mkdir(path.dirname(ledger), { recursive: true });
    await fsp.writeFile(ledger, `${JSON.stringify({ effective_plan_identity: { sha256: identity, canonical_json_utf8: canonical } }, null, 2)}\n`);
    const installed = spawnSync(bin, ["--json", "runtime", "install", "codex", "--root", target], { cwd: repo, env, encoding: "utf8" });
    assert.equal(installed.status, 0, `${installed.stdout}\n${installed.stderr}`);
    const checked = spawnSync(bin, ["--json", "doctor", "codex", "--skip-integration-verify"], { cwd: repo, env, encoding: "utf8" });
    ...
    const failed = spawnSync(bin, ["--json", "runtime", "update", "codex", "--root", target], { cwd: repo, env, encoding: "utf8" });
    ...
    assert.deepEqual((await fsp.readdir(home).catch(() => [])).sort(), homeBefore.sort(), "CLI must not mutate HOME");
  } finally {
    await Promise.all([fsp.rm(engineTar, { force: true }), fsp.rm(cliTar, { force: true })]);
  }
} finally {
