...
describe('public CLI architecture', () => {
  it('keeps a compact operator surface with owned active task state but no shadow planner/executor', () => {
  ...
  it('uses canonical source modules and has no retired command facades', () => {
    for (const file of ['install.ts', 'update.ts', 'rollback.ts', 'uninstall.ts', 'integration.ts', 'reference.ts', 'task.ts']) {
