...
4. Never require all three providers for ordinary UI work. Capability routing chooses only what the claim needs.
...
- Keep transient traces/screenshots/logs in tool-native temporary output unless the project or user requests a durable path; return a concise result.
