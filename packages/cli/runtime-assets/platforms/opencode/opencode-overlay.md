...
- Agent-rules installs only static `AGENTS.md`, skills and native MCP bytes.
  The dynamic routing plugin is retired; user plugins remain untouched.
- Installer proof is static native byte readback. Model-observed behavior is a
  separate host-owned claim.
