...
layers. This projection uses official profile loaders for static instructions and never
patches DSH source.
...
- Profile package install: `dsh plugin --profile <name> add <package>` (forwards to
...
- Hard tool policy: monotonic `ctx.tools.guard()` and the documented pre-execute
  waterfall; native approval, sandbox, scoped tools, MCP and headless
  profile.
