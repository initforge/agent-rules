...
export interface RequirementMapping {
  ...
  /**
   * Anchor for an effective requirement adopted after the immutable original
   * plan. This is intentionally separate from plan_anchor: core WorkLedger
   * anchors are bound to the immutable original bytes and must not be silently
   * re-bound to a later contract.
   */
...
interface LedgerShape {
  ...
  canonical_scope?: {
    reviewed_contract?: { path?: string; sha256?: string };
  };
}
...
export interface CompiledLedger {
  ...
  effectiveContract?: { path: string; sha256?: string };
}
...
export function readLedger(ledgerPath: string): CompiledLedger {
  ...
  // A compact execution ledger is not a plan-readiness ledger. Treating it as one
  // would silently compile zero requirements and can make an empty projection look
  // bounded-ready. Require the source requirement milestone that this compiler's
  // contract explicitly consumes, and fail closed with an actionable diagnosis.
  if (!raw.original_plan?.sha256 || !Array.isArray(raw.milestones?.M8?.requirements)) {
    throw new Error(`Ledger ${ledgerPath} is missing canonical original_plan.sha256 or milestones.M8.requirements; refusing zero-requirement readiness`);
  }
  ...
  return {
    ...
    effectiveContract: raw.canonical_scope?.reviewed_contract?.path
      ? {
          path: raw.canonical_scope.reviewed_contract.path,
          sha256: raw.canonical_scope.reviewed_contract.sha256,
        }
      : undefined,
  };
}
...
export function compileRequirements(
  ...
  // Requirements added by the adopted effective contract do not belong to the
  // immutable original plan. Keep their provenance explicit instead of
  // manufacturing an original-plan anchor or reporting a misleading metadata
  // gap. The contract hash is checked before any fallback anchor is accepted.
  const effectiveAnchorsByReq = new Map<string, NonNullable<RequirementMapping['effective_source_anchor']>>();
  const effectiveSourceTextByReq = new Map<string, string>();
  const effectiveContract = ledger.effectiveContract;
  if (effectiveContract?.path && repoRoot) {
    const candidate = path.resolve(repoRoot, effectiveContract.path);
    const repoPrefix = `${path.resolve(repoRoot)}${path.sep}`;
    if (candidate.startsWith(repoPrefix) && fs.existsSync(candidate)) {
      const contractText = fs.readFileSync(candidate, 'utf8');
      const contractSha = sha256(contractText);
      if (!effectiveContract.sha256 || effectiveContract.sha256 === contractSha) {
        const contractLines = contractText.split('\n');
        for (const req of ledger.m8Requirements) {
          if (anchorsByRequirementAlreadyDefined(ledger.anchors, req.id)) continue;
          const index = contractLines.findIndex((line) => line.includes(`id: ${req.id}`));
          if (index < 0) continue;
          effectiveAnchorsByReq.set(req.id, {
            source_path: effectiveContract.path,
            section_heading: 'contract.requirements',
            line_start: index + 1,
            line_end: index + 1,
            anchor_text_sha256: sha256(contractLines[index]) as Sha256,
          });
          effectiveSourceTextByReq.set(req.id, contractLines[index]);
        }
      }
    }
  }
  ...
  function anchorsByRequirementAlreadyDefined(anchors: CompiledLedger['anchors'], requirementId: string): boolean {
    return anchors.some((anchor) => anchor.requirement_id === requirementId);
  }
  ...
  for (const req of ledger.m8Requirements) {
    ...
    const effectiveSourceAnchor = effectiveAnchorsByReq.get(id) ?? null;
    const sectionText = anchor
      ? sectionTextFor(anchor.line_start, anchor.line_end)
      : effectiveSourceAnchor
        ? effectiveSourceTextByReq.get(id) ?? effectiveSourceAnchor.section_heading
        : '';
    ...
    const notes: string[] = [];
    if (!anchor && effectiveSourceAnchor) {
      notes.push(`effective source anchor recorded in adopted contract: ${effectiveSourceAnchor.source_path}:${effectiveSourceAnchor.line_start}`);
    } else if (!anchor) {
      notes.push('no plan anchor recorded in ledger');
    }
    ...
    // The canonical milestone status is authoritative for negative truth. A
    // GAP requirement may still have a plan anchor or acceptance criteria;
    // those describe intended work and must never be allowed to turn an
    // unimplemented requirement into PARTIAL in a projection/UI.
    const status: RequirementMapping['status'] = req.status === 'GAP' ? 'GAP'
      : req.status === 'SUPERSEDED' ? 'SUPERSEDED'
      : req.status === 'MATCH' && hasEvidence ? 'MATCH'
      : hasAnchor || hasAcs || hasEvidence ? 'PARTIAL'
      : 'GAP';

    out.push({
      requirement_id: id,
      source: anchor?.section_heading ?? effectiveSourceAnchor?.section_heading ?? 'M8 milestone requirement (no anchor)',
      status,
      plan_anchor: anchor ? {
      ...
      effective_source_anchor: effectiveSourceAnchor,
      acceptance_criteria: effectiveAcs,
      verification_profile: layerProfile(id, sectionText || id, anchor?.section_heading ?? effectiveSourceAnchor?.section_heading ?? 'ledger'),
  ...
  for (const mr of m11Reqs) {
    ...
    else if (presentModules.length === 0 && impl.tests.every((t) => !fs.existsSync(path.resolve(root, t)))) status = 'GAP';
    ...
    // Evidence = SHA-256 of module file bytes. Directories are
...
function buildSystemTopologyYaml(): Record<string, unknown> {
  ...
  return {
    ...
    queues: [{ id: 'ready-queue', status: 'EXISTS', path: 'packages/engine/src/dispatch-ready-set.ts [DELETED S5]', note: 'cross-stage typed ready queue from AM-0019 §4 (M11-C2)' }],
    migrations: [
...
function buildVerificationGraphYaml(requirements: RequirementMapping[], claims?: ClaimDefinition[]): Record<string, unknown> {
  return {
    ...
    requirements: requirements.map((r) => ({
      ...
      effective_source_anchor: r.effective_source_anchor ?? null,
...
export function compilePlanReadiness(input: PlanReadinessInput): PlanReadinessResult {
  ...
  // Tests and review tooling may write the nine projections into a temporary
  // directory while keeping the canonical original/contract in the repository.
  // Derive provenance from the original path when available; otherwise use the
  // normal `.agent/plans/<plan-id>` layout.
  const repoRoot = input.originalPath
    ? path.resolve(path.dirname(input.originalPath), '../../..')
    : path.resolve(input.planDir, '../../..');
...
// Claim semantics are independently maintained in claim-registry.ts. Re-exported
// here to preserve the established plan-readiness public API.
export {
  EVIDENCE_MATURITIES, CLAIM_FORMULAS, RISK_TIERS, MATURITY_RANK, FORMULA_THRESHOLDS,
  parseM11ClaimTitles, compileClaims, evaluateClaimFormulas,
  type EvidenceMaturity, type ClaimFormula, type RiskTier, type FreshnessDependency,
  type ClaimDefinition, type ClaimCompileInput, type ClaimEvidenceInput,
  type ClaimEvaluationResult, type ClaimFormulaResult, type ClaimFormulaSummary,
