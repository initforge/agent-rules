...
export interface PlanIdentity {
  ...
  /** Canonical plans use the shared manifest identity; old layouts are explicit compatibility only. */
  identityKind: 'canonical' | 'legacy-compatibility';
}
...
function deriveEffectiveIdentity(
  originalSha256: Sha256,
  amendments: readonly { id: string; sha256: Sha256 }[],
  legacyBytes: readonly Uint8Array[],
): { sha256: Sha256; kind: PlanIdentity['identityKind'] } {
  // A canonical-looking amendment must never silently fall back to the old
  // byte-concatenation algorithm. Gaps, duplicates, and tombstoned IDs are
  // rejected by the shared canonical identity compiler.
  const canonicalLayout = amendments.every(({ id }) => /^AM-\d{4}$/.test(id));
  if (canonicalLayout) {
    const identity = computeCanonicalEffectivePlanIdentity(
      originalSha256,
      amendments.map(({ id, sha256 }) => ({ amendment_id: id, sha256 })),
    );
    return { sha256: identity.sha256, kind: 'canonical' };
  }

  // Preserve old adopted projects during migration, but make the boundary
  // observable so this value cannot be mistaken for canonical protocol truth.
  return {
...
export function adoptPlan(sourcePath: string, planId: string, plansDir: string = AGENT_PLANS_DIR): PlanIdentity {
  ...
  const amendmentHashes: Array<{ id: string; sha256: Sha256 }> = [];
  const amendmentBytes: Uint8Array[] = [];
  for (const id of amendmentIds) {
    ...
    const bytes = fs.existsSync(amPath)
      ? fs.readFileSync(amPath)
      : fs.existsSync(amJsonPath)
        ? fs.readFileSync(amJsonPath)
        : fs.existsSync(amYamlPath)
          ? fs.readFileSync(amYamlPath)
          : undefined;
    if (bytes) {
      amendmentBytes.push(bytes);
      amendmentHashes.push({ id, sha256: sha256Bytes(new Uint8Array(bytes)) });
    }
  }

  const effective = deriveEffectiveIdentity(originalSha256, amendmentHashes, [rawBytes, ...amendmentBytes]);
