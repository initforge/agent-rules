...
/**
 * Small browser port used by the parity engine. Playwright is intentionally
 * loaded at runtime so the core engine can build on hosts that do not install
 * browser tooling. Invoking browser parity without the provider remains a hard
 * failure; this is dependency inversion, not a verification bypass.
 */
...
interface ConsoleMessagePort { type(): string; text(): string }
interface RequestPort { url(): string; failure(): { errorText?: string } | null; method(): string }
interface ResponsePort { request(): RequestPort; url(): string; status(): number }
interface LocatorPort {
  evaluate<R>(fn: (el: Element) => R | Promise<R>): Promise<R>;
}

export interface Page {
  on(event: 'console', listener: (msg: ConsoleMessagePort) => void): Page;
  on(event: 'pageerror', listener: (err: Error) => void): Page;
  on(event: 'requestfailed', listener: (req: RequestPort) => void): Page;
  on(event: 'response', listener: (res: ResponsePort) => void): Page;
...
interface AxeResultEntry { id: string; help: string; impact: string | null }
interface AxeAnalysis { violations: AxeResultEntry[]; incomplete: AxeResultEntry[] }
interface AxeBuilderPort {
  withTags(tags: string[]): AxeBuilderPort;
  analyze(): Promise<AxeAnalysis>;
}
type AxeBuilderConstructor = new (options: { page: Page }) => AxeBuilderPort;
let cachedAxeBuilder: AxeBuilderConstructor | null = null;

async function loadAxeBuilder(): Promise<AxeBuilderConstructor> {
  if (cachedAxeBuilder) return cachedAxeBuilder;
  const moduleName = '@axe-core/playwright';
  try {
    const mod = await import(moduleName) as Record<string, unknown>;
    const ctor = mod.AxeBuilder;
    if (typeof ctor !== 'function') throw new Error('AxeBuilder export missing');
    cachedAxeBuilder = ctor as AxeBuilderConstructor;
    return cachedAxeBuilder;
  } catch (error) {
    throw new Error(`browser accessibility provider unavailable: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function runParityManifest(manifest: ParityManifest, options: ParityRunOptions = {}): Promise<ParityRunResult> {
  ...
  let playwright: { chromium: { launch(options: { headless: boolean }): Promise<Browser> } };
  try {
    playwright = await import(moduleName) as typeof playwright;
  } catch (error) {
    throw new Error(`browser parity provider unavailable: ${error instanceof Error ? error.message : String(error)}`);
  }
