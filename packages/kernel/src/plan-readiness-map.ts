...
export const M11_IMPLEMENTATIONS: Record<string, M11Implementation> = {
  'M11-R11': {
    ...
    tests: ['packages/engine/test/plan-lifecycle.test.ts'],
    acceptanceCriteria: [
  ...
  'M11-R13': {
    ...
    status: 'SUPERSEDED',
    partialReason:
      'superseded: typed ready-set dispatch replaced by the sequential runner queue in the durable runtime',
  ...
  'M11-R22': {
    ...
    tests: ['platforms/claude/adapter.test.ts', 'platforms/opencode/adapter.test.ts', 'automation/host-attestation.test.ts'],
    acceptanceCriteria: [
  ...
  'M11-R23': {
    ...
    acceptanceCriteria: [
      'CERTIFICATION_REQUIRED_HOSTS = 9 canonical hosts enforced by workflow validation',
  ...
  'M11-R25': {
    ...
    modules: ['packages/engine/src/worker-adapter.ts'],
  ...
  'M11-R26': {
    ...
    acceptanceCriteria: [
      'controlled verification runs the adversarial closure suite and aggregates PASS/FAIL/WAITING_EXTERNAL',
  ...
  'M11-R27': {
    ...
    tests: ['packages/engine/test/evidence-dag.test.ts'],
    acceptanceCriteria: [
  ...
  'M11-R28': {
    ...
    tests: ['packages/engine/test/evidence-dag.test.ts', 'packages/engine/test/evidence-packet.test.ts'],
    acceptanceCriteria: [
  ...
  'M11-R37': {
    cluster: 'C5',
    status: 'SUPERSEDED',
    partialReason: 'superseded by truth-reset commit 6ed279b3: unconsumed MainRunCapsule/token-attribution leaf modules were removed; no runtime authority depends on them',
  ...
  'M11-R38': {
    cluster: 'C3',
    status: 'SUPERSEDED',
    partialReason: 'superseded by truth-reset commit 6ed279b3: MainRunCapsule premium-wake scheduling was removed as dead scheduling/context surface',
  ...
  'M11-R39': {
    cluster: 'C2',
    status: 'PARTIAL',
    partialReason: 'event-delta leaf was removed by truth reset; ArtifactPointer remains the active content-addressed primitive',
  ...
  'M11-R40': {
    cluster: 'C2',
    status: 'PARTIAL',
    partialReason: 'tool-output-broker leaf was removed by truth reset; ArtifactPointer remains active',
  ...
  'M11-R42': {
    cluster: 'C3',
    status: 'PARTIAL',
    partialReason: 'MainRunCapsule/content-cache-dedupe leaves were removed; context-cache remains active',
  ...
  'M11-R43': {
    cluster: 'C4',
    status: 'PARTIAL',
    partialReason: 'token-attribution leaf was removed; telemetry remains active but per-source attribution is not fully proven',
  ...
  'M11-R44': {
    cluster: 'C4',
    status: 'PARTIAL',
    partialReason: 'content-cache-dedupe leaf was removed; context-cache remains active',
  ...
  'M11-R45': {
    cluster: 'C2',
    status: 'PARTIAL',
    partialReason: 'tool-output-broker leaf was removed; ArtifactPointer drill-down remains active',
    modules: ['packages/engine/src/tool-output-broker.ts', 'packages/engine/src/artifact-pointer.ts'],
    tests: ['packages/engine/test/tool-output-broker.test.ts', 'packages/engine/test/artifact-pointer.test.ts'],
    acceptanceCriteria: [
