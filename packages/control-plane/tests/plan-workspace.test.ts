...
function writeFixture(root: string, planId: string, overrides?: {
  ...
  const anchor = { planSha256: origSha, sectionHeading: 'Requirement 1', lineStart: 4, lineEnd: 4,
    anchorTextSha256: sha('Do the work.\n'), requirementId: 'REQ-001', chunkIndex: 0 }
  ...
  const aid = crypto.createHash('sha256').update(Buffer.from([anchor.planSha256, anchor.sectionHeading, String(anchor.lineStart), String(anchor.lineEnd), anchor.anchorTextSha256, anchor.requirementId, String(anchor.chunkIndex)].join(':'), 'utf-8')).digest('hex')
