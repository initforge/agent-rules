...
describe('API', () => {
  ...
  it('GET /api/config/platforms returns platform configs', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
    expect(Object.keys(res.body.data).length).toBeGreaterThanOrEqual(4);
  });
  ...
  it('POST /api/mutation/diff computes diff', async () => {
    const res = await request(app)
      ...
      .send({ filePath: 'automation/model-policy.json', content: JSON.stringify({ version: 4, platforms: {} }) });
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
  ...
  it('POST /api/mutation/preview returns diff before apply', async () => {
    ...
    const res = await request(app)
      ...
      .send({ target: 'model-policy', filePath, data: { version: 4, platforms: {} } });
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
