...
- `01-global/loi/`: platform-neutral always-loaded behavior.
- `01-global/ky-nang/`: lazy procedures grouped by subsystem.
- `02-du-an/`: project context and templates.
- `01-global/tich-hop/`: external tools, manifests, adapters and policy.
- `03-nen-tang/<name>/`: only platform delta.
- `04-tu-dong-hoa/`: build/install/validation/sync guards.
...
Reverse-sync guard: never copy installed runtime/admin/system skills back into `01-global/ky-nang`. External/runtime-only skills stay outside canonical source unless the owner explicitly promotes their behavior into a small canonical rule or renamed project-owned capability.
