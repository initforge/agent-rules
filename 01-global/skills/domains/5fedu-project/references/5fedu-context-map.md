...
- Project layer: repo-local `AGENTS.md` plus canonical `context/5fedu/AGENTS.md` and subfolders `00-map`, `10-live-rules`, `20-mapping`, `30-evidence`, `40-legacy`, `90-notes`. `.agents/AGENTS.md` and `.codex/AGENTS.md` are thin pointers only; never mirror the full pack into platform folders.
...
Every 5fedu repo should track decisions in `context/5fedu/00-map/decisions.md`; when legacy expanded files exist, keep `context/5fedu/40-legacy/decision-status-legacy.md` synchronized too.
