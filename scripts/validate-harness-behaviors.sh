...
GROK_RULES="${GROK_HOME:-$HOME/.grok}/.grok/rules/00-hard-activation-contract.md"
if [[ -f "$GROK_RULES" ]]; then
  grep -q 'Turn-0 Skill Scan' "$GROK_RULES" || fail "global Grok rules missing Turn-0 — run install-grok-global.sh"
  grep -q 'Visible Echo Contract' "$GROK_RULES" || fail "global Grok rules missing Visible Echo — run install-grok-global.sh"
  ok "global Grok rules have Turn-0 + Visible Echo"
  for skill in e2e-qa researcher product-ui-craft 5fedu-project docs-style; do
...
STALE=$(grep -rE 'frontend-ui-quality|taste-skill|ui-ux-pro-max' \
  codex/rules \
  $(find codex/skills -maxdepth 2 -name '*.md' ! -path '*/_archive/*' 2>/dev/null) 2>/dev/null \
...
if command -v grok >/dev/null 2>&1; then
  ...
  HAS_CRAFT=$(echo "$INSPECT" | grep -c 'product-ui-craft' || true)
  ...
  if [[ "$HAS_CRAFT" -lt 1 ]]; then
    fail "grok inspect missing product-ui-craft skill"
  else
    ok "grok inspect lists product-ui-craft"
  fi
