...
function checkState() {
  const statePath = path.join(AGENT_DIR, 'state', 'current.json');
  if (!existsSync(statePath)) {
    warn('.agent/state/current.json missing — no active plan');
    return null;
  }
  let pointer;
  try {
    pointer = JSON.parse(readFileSync(statePath, 'utf8'));
  } catch (e) {
    fail(`.agent/state/current.json is not valid JSON: ${e.message}`);
    return null;
  }
  for (const key of ['plan_id', 'plan_root', 'requirements']) {
    if (!pointer[key]) fail(`.agent/state/current.json missing "${key}"`);
  }
