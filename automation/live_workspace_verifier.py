...
def verify(workspace: Path, response_path: Path, fixture: dict[str, Any]) -> dict[str, Any]:
    ...
    correctness_ok = all(file_checks) and all(command_checks)
    if not file_checks and not command_checks:
        correctness_ok = scope_ok and communication_ok
    verification_ok = all(command_checks) if command_checks else scope_ok
...
def main() -> int:
    ...
    try:
        ...
        result = verify(args.workspace.resolve(), args.response.resolve(), fixtures[args.fixture])
