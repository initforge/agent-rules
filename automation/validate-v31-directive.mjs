...
const ledger = json('.agent/ledger/v3.1-external-first.json');
...
check('raw-intent', original.startsWith('# AGENT-RULES V3.1') && originalHash === pointer.original.sha256, `raw directive hash ${originalHash}`);
...
check('current-authority', pointer.work_id === 'v3.1-external-first' && pointer.plan_id === pointer.work_id && pointer.generation >= 4, `generation=${pointer.generation}, work_id=${pointer.work_id}`);
check('ledger-binding', pointer.canonical_ledger.path === '.agent/ledger/v3.1-external-first.json' && pointer.canonical_ledger.sha256 === sha(pointer.canonical_ledger.path), 'active ledger is hash-bound');
check('contract-binding', pointer.contract.path === '.agent/plans/v3.1-external-first/requirements.yaml' && pointer.contract.sha256 === sha(pointer.contract.path), 'requirements contract is hash-bound');
check('ledger-identity', ledger.effective_plan_identity?.sha256 === '963ec943c23edc414cd1512f10bc13e153a27b194cb49836b1a495edb1aae2d6' && crypto.createHash('sha256').update(ledger.effective_plan_identity.canonical_json_utf8).digest('hex') === ledger.effective_plan_identity.sha256, 'effective plan identity is self-verifying');
