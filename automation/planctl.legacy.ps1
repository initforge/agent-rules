...
try {
  switch ($Action) {
    ...
    "handoff" {
      $compiled = Require-Compiled
      if ($HandoffMode -eq "executor") {
        $dir = Get-StateDirectory $compiled.plan_id
        $out = if ($OutputPath) { $OutputPath } else { Join-Path $dir "EXECUTOR-HANDOFF.md" }
        $phaseLines = @($compiled.phases | ForEach-Object {
          "- $($_.id): $($_.title) | files: $($_.files_touched -join '; ') | verify: $($_.verify_gate)"
        }) -join "`n"
        $text = @"
---
HANDOFF - Plan ID: $($compiled.plan_id)
Role: executor
Model lane: economical implementation agent
Authority: execute the approved plan only; do not re-plan or expand scope
State: .agent/plans/$($compiled.plan_id)/state.json
Plan source: $PlanPath
Execution mode: $($compiled.execution_mode)
Verification strategy: $($compiled.verification_strategy)

Phases:
$phaseLines

Operating rules:
- Start the next unfinished phase with planctl before editing; keep its lease.
- Implement only the listed deliverables and files for that phase.
- Record command output as evidence, then complete the phase through planctl.
- Stop as BLOCKED when a required decision, permission, or proof is missing.
- Forbidden: scope creep; silent plan rewrite; bypassing verification; claiming PASS without evidence.
---
"@
        $target = if ([IO.Path]::IsPathRooted($out)) { $out } else { Join-Path (Get-Location) $out }
        $target = Assert-SafePath $target "Executor handoff output path"
        [IO.File]::WriteAllText($target, $text, [Text.Encoding]::UTF8)
        Write-Host "HANDOFF_READY: generated executor handoff at $out"; exit 0
      }
      $phaseId = if ($Phase) { $Phase } else { $compiled.phases[0].id }
