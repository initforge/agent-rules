...
export async function resolveNativeExecutable(
  ...
  switch (host) {
    ...
    case 'antigravity':
      ...
      candidates.push(...pathCommandCandidates('gemini', platform, env));
      break;
    case 'cursor':
      candidates.push(path.join(homeDir, '.cursor', 'bin', platform === 'win32' ? 'cursor.exe' : 'cursor'));
      candidates.push(...pathCommandCandidates('cursor', platform, env));
