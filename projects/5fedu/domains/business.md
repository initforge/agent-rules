...
## 9. Organizational baseline: Nhân viên → Phòng ban → Chức vụ

Với nhóm module quản trị nội bộ, không được suy luận mỗi module là một thế giới riêng.

- **Nhân viên** là baseline entity/module gốc cho CRUD, form, detail, toolbar, export và stats shell.
- **Phòng ban** là trục tổ chức cha; mặc định chỉ hierarchy 2 cấp trừ khi spec/template/app xác nhận sâu hơn.
- **Chức vụ** nằm trong trục **Phòng ban**; khi thêm/sửa/lọc/grouping/permission scope, xem **Phòng ban** là parent context.
- Module mới cùng họ quản trị nội bộ → clone/adapt từ **Nhân viên**, chỉ đổi khác biệt nghiệp vụ.
- Không tách **Chức vụ** thành pattern độc lập làm lệch dropdown, detail, filter và quan hệ tổ chức.

Ví dụ: form Nhân viên chọn Chức vụ để suy ra Phòng ban; detail Phòng ban hiện quan hệ xuống Chức vụ; list Chức vụ lọc theo cây Phòng ban.

## 10. Stats shell reuse

Thống kê là pattern sống riêng, không phải mỗi module tự vẽ lại.

- Module có **Thống kê** → reuse shell stats **Nhân viên**: tab stats, toolbar lọc, KPI, chart, grid, drilldown, export report.
- Chỉ thay source data, label nghiệp vụ, công thức và permission scope theo module.
- Không chèn mini-stats tạm vào CRUD page nếu nghiệp vụ cần surface stats/report riêng.
- Export/print stats dùng cùng data đã filter/tính với UI stats.

Ví dụ: thống kê module mới vẫn theo shell Nhân viên, thay KPI/chart/bảng tổng hợp và tên file xuất theo domain.
